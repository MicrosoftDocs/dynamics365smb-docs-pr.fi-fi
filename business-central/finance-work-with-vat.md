...
description: 'Tässä artikkelissa kuvataan erilaisia tapoja käsitellä ALV:tä sekä manuaalisesti että automaattisten asetusten avulla, jotta voit noudattaa maa- ja aluekohtaisia säädöksiä.'
author: brentholtorf
ms.author: bholtorf
...
## ALV-summien laskeminen ja näyttäminen myynti- ja ostoasiakirjoissa  
...
Voit laskea ja näyttää arvonlisäveron määrät myynti- ja ostotositteissa eri tavalla. Ero riippuu asiakkaan tai toimittajan tyypistä, jota käsittelet. Voit myös muuttaa tietylle tapahtumalle lasketun ALV-summan manuaalisesti, esimerkiksi siten, että toimittajan laskemaa ALV-summaa käytetään.

### ALV:n sisällyttäminen hintoihin ja rivisummiin sekä sen poissulkeminen
...
#### Liiketoiminnan ALV-kirjausryhmien ja asiakashintaryhmien käyttäminen 
...
* Jos haluat käyttää jotain ALV-prosenttia tietylle asiakasryhmälle, voit valita ryhmän **Asiakashintaryhmä**-sivun **Liiketoiminnan ALV-kirjaus ryhmä (hinta)** -kentässä. Tämä asetus on hyödyllinen esimerkiksi silloin, kun haluat, että hinta koskee kaikkia asiakkaita tietyllä maantieteellisellä alueella tai tietyllä toimialalla.
...
## ALV-päivämäärän käyttäminen

### ALV-päivämäärä asiakirjoissa
...
> [!IMPORTANT]
> Versiosta 23.1 alkaen tietyn päivämääräalueen **ALV-päivämäärän** kirjaus voidaan sallia tai estää käyttämällä **ALV-asetukset**- ja **Käyttäjäasetukset**-sivujen **Salli ALV-päivämäärästä alkaen**- ja **Salli ALV-päivämäärään asti** -kenttiä. Jos käytössä on tätä vanhempi versio, tietyn päivämääräalueen **ALV-päivämäärän** kirjaus voidaan sallia tai estää käyttämällä **Pääkirjanpidon asetukset**- ja **Käyttäjäasetukset**-sivujen **Ensimm. sallittu kirjauspvm:**- ja **Viimeinen sallittu kirjauspvm:** -kenttiä.  
...
### ALV-päivämäärän muokkaaminen kirjatuissa tapahtumissa
...
## Myynti- ja ostoasiakirjojen ALV-summien oikaisu manuaalisesti  
...
Jos maksualennus on laskettu ALV:n sisältävän laskusumman perusteella, ALV-summan maksualennusosa peruutetaan, kun maksualennus annetaan. Sinun tulee aktivoida **Muutos maksualennusten osalta** -kenttä sekä pääkirjanpidon asetuksissa (yleisesti) että ALV-kirjausasetuksissa (erityisille liiketoiminnan ALV-kirjausryhmien ja tuotteen ALV-kirjausryhmien yhdistelmille).  

### Järjestelmän määrittäminen myyntiasiakirjojen manuaalisille ALV-tapahtumille

Seuraa näitä ohjeita ottaaksesi manuaalisen ALV-muutoksen käyttöön myyntiasiakirjoissa. **Ostojen ja ostovelkojen asetukset** -sivun vaiheet ovat samankaltaiset.

1. Määritä **Pääkirjanpidon asetukset** -sivulla sovelluksen laskeman summan ja manuaalisen summan **Maksimi sallittu ALV-ero**.  
2. Lisää **Myyntien ja myyntisaamisten asetukset** -sivulla valintamerkki **Salli ALV-ero** -valinta.  

### Myyntiasiakirjan ALV:n muuttaminen:

1. Avaa asianomainen myyntitilaus.  
...
## ALV:n laskeminen manuaalisesti päiväkirjojen avulla  

Voit oikaista ALV-summia myös at yleisessä päiväkirjassa sekä myynti- ja ostopäiväkirjoissa. Saatat tarvita oikaisun esimerkiksi, jos syötät päiväkirjaasi toimittajan laskun ja [!INCLUDE[prod_short](includes/prod_short.md)] -ohjelman laskema ALV-summa ja toimittajan laskun ALV-summa eroavat.  

### Järjestelmän määrittäminen yleisen päiväkirjan manuaalisille ALV-tapahtumille
...
### Järjestelmän määrittäminen myynti- ja ostopäiväkirjojen manuaalisille ALV-tapahtumille
...
3. Kun asetukset ovat valmiit, voit oikaista yleisen päiväkirjan rivin **ALV-summa**-kentän tai myynti- tai ostopäiväkirjan rivin **Vastatilin ALV-summa** -kentän. [!INCLUDE[prod_short](includes/prod_short.md)] vahvistaa, että ero ei ole suurempi kuin määritetty enimmäisarvo.  
...
## Tuonnin ALV:n kirjaaminen ostolaskuissa
Tuontia koskevan ALV-laskun voi kirjata ostolaskulla päiväkirjojen asemesta.  

### Määritä ostaminen tuonnin ALV-laskujen kirjaukseen
...
7. Luo ALV-viranomaisen **Ylein. liiketoim. kirjausryhmän** ja tuontia koskevan ALV:n **Yleinen tuotteen kirjausryhmän** yhdistelmä. Valitse **Ostotili**-kentässä tuontia koskevan ALV:n KP-tili tätä yhdistelmää varten.  

### Kun olet tehnyt määritykset valmiiksi, luo uusi lasku tuontiviranomaiseksi määritetylle toimittajalle  
...
## Tarjontasertifikaattien käsitteleminen
...
### Tarjontasertifikaatin tietojen katseleminen.  
...
4. Jotta voit sisällyttää tiedot sertifikaatin toimitusasiakirjan riveiltä, ota käyttöön **Tulosta rivitiedot** -valitsin.  
5. Jos haluat, että [!INCLUDE[prod_short](includes/prod_short.md)] luo varmenteita lähetetyille lähetyksille, joilla ei ollut niitä, laita päälle **Luo toimitusvarmenteita, jos niitä ei ole vielä luotu** -valitsin. Kun otat käyttöön valitsimen, kaikille niille kirjatuille toimituksille luodaan uudet sertifikaatit, joilla ei ole sertifikaatteja valitulla alueella.  
...
### Tilauksen tarjontasertifikaatin tilan päivittäminen  
...
Voit tarkastella sertifikaattiryhmiä aloittamalla **Tarjontasertifikaatit**-sivulta ja päivitä sitten avointen sertifikaattien tila, kun vastaanotat ne asiakkaalta. Päivitystiedoista voi olla hyötyä, jos haluat etsiä kaikki tietyn tilan omaavat sertifikaatit, esimerkiksi **Pakollinen**, ja joille haluat päivittää tilaksi **Ei vastaanotettu**.  

### Tilauksen tarjontasertifikaattien ryhmän tilan päivittäminen  
...
> [!NOTE]
> Et voi luoda uutta tarjontasertifikaattia **Tarjontasertifikaatit**-sivulla, kun siirryt siihen tällä menetelmällä. Voit luoda sertifikaatin toimitukselle, jota ei asetettu vaatimaan sitä avaamalla kirjatun myyntitoimituksen, ja käyttämällä yhtä kahdesta aiemmin kuvatusta toimenpiteestä:  
>
