...
Myyjää pidetään usein liiketoiminnan eniten ulospäin suuntautuneena tehtävänä. Myyjien voisi kuitenkin olla hyödyllistä tarkastella liiketoimintaa myös sisäisesti, jotta he tiedostaisivat, mitä taustalla tapahtuu. Myyjät saavat merkityksellisiä tietoja, kun [!INCLUDE[prod_short](includes/prod_short.md)] ja [!INCLUDE[crm_md](includes/crm_md.md)] integroidaan. Integrointi mahdollistaa sen, että ihmiset voivat tarkastella tietoja [!INCLUDE[prod_short](includes/prod_short.md)] työskennellessään [!INCLUDE[crm_md](includes/crm_md.md)]. Myyntitarjousta valmistellessa voi esimerkiksi olla hyödyllistä tietää, riittääkö varasto tilauksen täyttämiseen. Lisätietoja on kohdassa [Dynamics 365 Salesin käyttäminen Business Centralissa](marketing-integrate-dynamicscrm.md).

> [!NOTE]
> Tässä artikkelissa kuvataan online-versioiden [!INCLUDE[crm_md](includes/crm_md.md)]  [!INCLUDE[prod_short](includes/prod_short.md)] integrointiprosessi [!INCLUDE[prod_short](includes/cds_long_md.md)]. Lisätietoja paikallisesta määrityksestä on kohdassa [Paikallisen Dynamics 365 Salesin integroinnin valmistelu](/dynamics365/business-central/dev-itpro/administration/prepare-dynamics-365-for-sales-for-integration).
...
| Kenttä | Kuvaus |
|--|--|
| **Tuo Dynamics 365 Sales -ratkaisu** | Asenna ja määritä integrointiratkaisu [!INCLUDE[crm_md](includes/crm_md.md)] -sovelluksessa. |
...
|**Myyntitilausten kaksisuuntainen synkronointi**|Synkronoi myyntitilaukset molempiin suuntiin. Lisätietoja tästä vaihtoehdosta [on myyntitilausten yksi- ja kaksisuuntaisessa synkronoinnissa](#single-and-bi-directional-synchronization-of-sales-orders).<br><br>**Huomautus:** Et voi käyttää tätä vaihtoehtoa, jos käytössä on **Ota käyttöön vanha myyntitilauksen integrointi** -vaihtoehto. Nämä kaksi asetusta ovat toisensa poissulkevia.|

> [!TIP]
> Jos haluat myös integroitua Microsoft Dynamics 365 Field Service, oppaassa on valinnainen vaihe, josta voi olla apua. Saat lisätietoja Field Service -integroinnista, kun siirryt Integrointi [-ohjelmaan Microsoft Dynamics 365 Field Service](admin-integrate-field-service.md).
...
|**Synkronoi nimikkeen saatavuus automaattisesti**|Määrittää, että nimikkeen saatavuuden työjono on ajoitettava. Työjono suoritetaan 30 minuutin välein, ja se päivittää yhdistettyjen nimikkeiden saatavuuden.|
|**Dynamics 365 SDK -versio**|Jos integroit yrityksen paikallisen version [!INCLUDE[crm_md](includes/crm_md.md)] kanssa, käytä Dynamics 365 -ohjelmiston kehityspakettia (jota kutsutaan myös nimellä Xrm) yhteyden muodostamiseen [!INCLUDE[prod_short](includes/prod_short.md)]  [!INCLUDE[crm_md](includes/crm_md.md)]. Valitsemasi version on oltava yhteensopiva [!INCLUDE[crm_md](includes/crm_md.md)]in käyttämän SDK-version kanssa. Tämä versio on sama tai uudempi kuin [!INCLUDE[crm_md](includes/crm_md.md)]in käyttämä versio.|

Näiden asetusten lisäksi voit määrittää seuraavat asetukset [!INCLUDE[crm_md](includes/crm_md.md)].
...
Kun määrität integroinnin, käytettävissä on vaihtoehtoja, jotka ohjaavat myyntitilausten synkronointisuuntaa ja lähetystapoja.

**Myyntitilausten kaksisuuntainen synkronointi** -vaihtoehdon avulla voit synkronoida myyntitilaukset Salesista [!INCLUDE [prod_short](includes/prod_short.md)] -sovellukseen ja päin vastoin. Jos asiakas esimerkiksi muuttaa mieltään tuotteesta tai määrästä [!INCLUDE[crm_md](includes/crm_md.md)], voit arkistoida myyntiasiakirjan ja luoda uuden asiakirjan [!INCLUDE[prod_short](includes/prod_short.md)]. Sama pätee muutoksiin [!INCLUDE[prod_short](includes/prod_short.md)]issa. Esimerkiksi silloin, kun hinnat, verosummat tai oletetut toimituspäivämäärät muuttuvat, muutokset synkronoidaan [!INCLUDE[crm_md](includes/crm_md.md)] -sovellukseen. Kaksisuuntainen synkronointi auttaa pitämään myyjät ajan tasalla uusimmista muutoksista ja myyntitilausten tilasta.
...
| Myyntitilauksen otsikko | Myyntitilaus | [!INCLUDE[prod_short](includes/prod_short.md)] -> [!INCLUDE[crm_md](includes/crm_md.md)] ja [!INCLUDE[crm_md](includes/crm_md.md)] -> [!INCLUDE[prod_short](includes/prod_short.md)] <br><br> Voit synkronoida molempiin suuntiin ottamalla **Myyntitilausten kaksisuuntaisen synkronoinnin** -vaihtopainikkeen käyttöön **Dynamics 365 -yhteyden asetus** -sivulla.| [!INCLUDE[prod_short](includes/prod_short.md)] Myynnin tunnistetietojen suodatus: **Asiakirjatyyppi** on **Tilaus** ja **Tila** vapautettu **·** |
...
Kullekin yksikköryhmälle voidaan luoda useita mittayksiköitä, ja nämä ryhmät voidaan määrittää tuotteisiin [!INCLUDE[crm_md](includes/crm_md.md)]issa. Myöhemmin voit synkronoida tuotteet kohteiden ja resurssien [!INCLUDE[prod_short](includes/prod_short.md)] kanssa. Nimikkeen mittayksiköt ja resurssiin mittayksiköt voidaan yhdistää manuaalisesti yksikköryhmään. Jos nimikkeen tai resurssin yksikköryhmää ei ole liitetty yksikköryhmään [!INCLUDE[crm_md](includes/crm_md.md)] esimerkiksi sen takia, että yksikköryhmää ei ollut olemassa, [!INCLUDE[prod_short](includes/prod_short.md)]  yksikköryhmä [!INCLUDE[crm_md](includes/crm_md.md)] luodaan automaattisesti tähän ryhmään.
...
2. Koska uusia tuotteita luodaan [!INCLUDE[crm_md](includes/crm_md.md)] uusien yksikköryhmien avulla, voit välttää nimien kaksoiskappaleet tekemällä jonkin seuraavista vaiheista:
...
|Nimikkeet|Kun nimikkeitä synkronoidaan tuotteiden kanssa [!INCLUDE[crm_md](includes/crm_md.md)] , [!INCLUDE[prod_short](includes/prod_short.md)]  ohjelma luo automaattisesti hinnaston [!INCLUDE[crm_md](includes/crm_md.md)]. Voit välttää synkronointivirheet, jos et muokkaa tätä hinnastoa manuaalisesti.|
...
|Myyntitilaukset|Kun otat myyntitilausten integroinnin käyttöön, kun vapautat myyntitilauksia [!INCLUDE[prod_short](includes/prod_short.md)] , jotka on luotu lähetetyistä myyntitilauksista [!INCLUDE[crm_md](includes/crm_md.md)] , ne synkronoidaan [!INCLUDE[crm_md](includes/crm_md.md)] ohjelman myyntitilausten kanssa. Ennen tilausten synkronointia on suositeltavaa synkronoida kaikki tilaukseen liittyvät taulukot. Esimerkiksi myyntihenkilöt ja hintaluettelot. Tilauksen otsikossa oleva Myyjän koodi -kenttä määrittää yhdistetyn taulukon omistajan [!INCLUDE[crm_md](includes/crm_md.md)]:ssä.|
...
Tiimi Microsoft Power Platform  [ilmoitti](/power-platform/important-changes-coming#deprecation-of-office365-authentication-type-and-organizationserviceproxy-class-for-connecting-to-dataverse) , että se vähättelee todennuksen tyyppiä Office 365 . Jos käytät [!INCLUDE[prod_short](includes/prod_short.md)] yrityksen versiota, joka on aikaisempi kuin [!INCLUDE [prod_short](includes/prod_short.md)] 2019 julkaisuaalto 1, muodosta online-yhteys [!INCLUDE[crm_md](includes/crm_md.md)] OAuth-todennustyypin avulla. Tässä osassa kerrotaan, miten seuraavat tuoteversiot yhdistetään:
...
* Sinulla täytyy olla Microsoft Azure -tilaus. Kokeilutili toimii sovelluksen rekisteröinnin yhteydessä.
