...
Käytä **Omat asetukset** -sivua hallitaksesi perusasetuksia [!INCLUDE[prod_short](includes/prod_short.md)] -ohjelmassa. Tekemäsi muutokset vaikuttavat vain omaan työtilaasi; ei muiden käyttäjien työtiloihin.  
...
Voit asentaa lisäkieliä sovellukseen [!INCLUDE[prod_short](includes/prod_short.md)] AppSourcesta. Kaikki tuetut näyttökielet näkyvät luettelossa. Järjestelmänvalvojan on asennettava sopiva kielisovellus vuokraajaan, ennen kuin käyttäjät voivat vaihtaa uuteen kieleen [!INCLUDE[prod_short](includes/prod_short.md)]issa.  
...
## <a name="notifications"></a>Ilmoitukset ( ei mainos )

Valitse **Muuta, kun saan ilmoituksia** -linkin hallitaksesi ilmoituksia, joita saat tietyistä tapahtumista tai tilamuutoksista. Voit esimerkiksi saada ilmoituksen, kun olet laskuttamassa asiakasta, jolla on erääntynyttä saldoa, tai kun käytettävissä oleva varasto on pienempi kuin myytävä määrä. Lisätietoja on kohdassa [Ilmoitusten hallinta](ui-smart-notifications.md).
