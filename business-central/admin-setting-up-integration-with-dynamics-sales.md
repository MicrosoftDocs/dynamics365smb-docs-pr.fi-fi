...
> [!IMPORTANT]
> Asennuksen aikana sinua pyydetään antamaan tunnistetiedot [!INCLUDE[prod_short](includes/cds_long_md.md)] -ympäristölle. Toimita sellaisen tilin valtuudet, joka on lisensoitu käyttäjä ja joka on määritetty järjestelmänvalvojan **suojausrooliin** [!INCLUDE[prod_short](includes/cds_long_md.md)] ympäristössä ja [käyttäjän järjestelmänvalvojan](/entra/identity/role-based-access-control/permissions-reference#user-administrator) rooliin vuokraajassa, johon ympäristö kuuluu. Tämä tili ei tarvitse käyttöoikeutta [!INCLUDE[prod_short](includes/prod_short.md)] -ohjelmaan, koska sitä käytetään vain tehtävien määrittämiseen [!INCLUDE[prod_short](includes/cds_long_md.md)] -ympäristössä.
>
