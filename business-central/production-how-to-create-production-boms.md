...
Tuotannon tuoterakenne sisältää perustiedot nimikkeen tuotannossa käytettävistä komponenteista ja osakokoonpanoista. Kun luot päänimikkeelle tuotantotilauksen, tuotannon tuoterakenne ohjaa materiaalitarpeiden laskentaa **Tuot.til. komponentit** -sivulla näkyvällä tavalla.

[!INCLUDE[prod_short](includes/prod_short.md)] tukee myös kokoonpanon tuoterakenteita. Käytä kokoonpanotilauksia valmistaaksesi loppunimikkeitä komponenteista yksinkertaisessa prosessissa, jonka yksi tai useampi perusresurssi voi suorittaa ja jotka eivät ole kone- tai tuotantosoluja. Tai prosessi, joka voidaan suorittaa ilman resursseja. Kokoonpanoprosessi voi olla esimerkiksi kahden viinipullon ja yhden kahvipaketin valinta ja niiden pakkaaminen lahjaksi. Lisätietoja on kohdassa [Kokoonpanon tuoterakenteet tai tuotannon tuoterakenteet](inventory-how-work-boms.md#assembly-boms-or-production-boms).  
...
Käytä esimerkiksi Tuotannon tuoterakenteen uusia versioita, käyn lokasoijan esimerkiksi silloin, kun nimike vaihdetaan toiseen nimikkeeseen tai silloin, kun asiakas pyytää tuotteesta erikoisversiota. Versioperiaate mahdollistaa tuotannon tuoterakenteen useiden versioiden hallinnan. Tuotannon tuoterakenteen version rakenne vastaa tuotannon tuoterakenteen rakennetta. Perusero on versioiden ajallinen voimassaolo. Voimassaolon päivämäärä määrittää aloituspäivämäärä.  
...
4. Täytä tarvittavat kentät. [!INCLUDE [tooltip-inline-tip_md](includes/tooltip-inline-tip_md.md)]
...
Laskentakaava määrittää yksittäisten komponenttien suhteen. Laskentakaavaksi ovat saatavilla seuraavat vaihtoehdot:  
...
Tuotannon tuoterakenne vaatii 70 metalliosaa, joiden dimensioiden pituus = 0,20 m ja leveys = 0,15 m. Arvot annetaan seuraavalla tavalla: Laskentakaava = Pituus x Leveys, Pituus = 20, Leveys = 15, Määrä per = 70.

Määrä per x Pituus * Leveys eli Määrä = 70 x 0,20 m x 0,15 m = 2,1 m2 antaa määrän.  
