...
- Tililläsi Microsoft 365 on ainakin [Exchange Administrator](/entra/identity/role-based-access-control/permissions-reference#exchange-administrator] -rooli.
...
1. Valitsemalla **Outlook-lisäohjelman keskitetty käyttöönotto** voit käynnistää avustetun asennusoppaan.
1. Tarkista ensimmäinen sivu ja avaa sivu apuohjelmien lataamista varten valitsemalla **Seuraava**.
1. Valitse **Käyttöönotto**-sarakkeesta käyttöönotettavien apuohjelmien valintaruutu ja valitse sitten **Lataa ja jatka**.

    Laitteeseen ladataan tiedosto, jonka nimi on *OutlookAddins.zip*

1. Tässä vaiheessa olet suorittanut kaiken Business Centralissa, joten voit valita **Valmis**.
...
1. Siirry kansioon, johon OutlookAddins.zip on ladattu, ja pura **Contact Insights.xml**- ja **Document View.xml**-tiedostot haluamaasi kansioon.
...
1. Kirjaudu Microsoft 365 -hallintakeskukseen ja valitse sitten [Integroidut sovellukset](https://go.microsoft.com/fwlink/?linkid=2163967).

1. Valitse **Lataa mukautetut sovellukset**.
1. **Määritä Käyttöönotettavat sovellukset** -sivulla **sovelluksen tyypiksi** **Office-lisäosa**.
1. Valitse **Lataa luettelotiedosto (.xml) laitteesta** > **Valitse tiedosto**.
1. Valitse lisätiedostoista **Content Insights.xml** tai **Documents.xml**, jotka purit aiemmin.
1. Määritä käyttäjät ja ota apuohjelma käyttöön noudattamalla ohjeita.
1. Toista vaiheet 9-11 halutessasi muille apuohjelmille.
