...
**Työjonon tapahtumat** -sivulla on luettelo kaikista aiemmin luoduista töistä. Jos lisäät työjonotapahtuman, suoritetaan ajoitettuna, sinun on annettava joitakin tietoja. Esimerkki:

* Suoritettavan objektin tyyppi, kuten raportti tai codeunit. Sinulla on oltava oikeus suorittaa raportti tai codeunit.
...
* Aikataulu sille, milloin ja kuinka usein työjonotapahtuma suoritetaan.
...
Kun työ on valmis, [!INCLUDE [prod_short](includes/prod_short.md)] poistaa sen työjonotapahtumien luettelosta, ellei se ole toistuva työ. Toistuvien töiden yhteydessä **Aloituspvm ja -aika** -kentän arvo muokataan näyttämään seuraava aika, jolloin työ suoritetaan.
...
| Pidossa | Työtapahtuman oletustila, kun se luodaan. Valitse **Aseta tilaksi valmis** -toiminto, jotta tilaksi muuttuu **Valmis**. Palauta tilaksi **Pidossa** valitsemalla **Määritä pitoon**. Lisätietoja: [Tietoa Pidossa-tilasta](#about-on-hold).|
| Estossa toimettomuuden vuoksi | Käytetään pääasiassa työjonon tapahtumille, jotka ajoittavat synkronoinnin sovelluksen [!INCLUDE [prod_short](includes/prod_short.md)] ja toisen sovelluksen, kuten [!INCLUDE [cds_long_md](includes/cds_long_md.md)], kanssa. Lisätietoja tästä tilasta on kohdassa [Tietoja toimettomuuden aikakatkaisuista](/dynamics365/business-central/admin-scheduled-synchronization-using-the-synchronization-job-queue-entries#about-inactivity-timeouts). |
|Odottaa | Koskee vain työjonotapahtumia, joille määritetään luokkakoodi. Ilmaisee, että työ on ajoitettu, mutta sen taustalla oleva ajoitettu tehtävä ei ole aktiivinen. Suoritettavana olevan saman luokan työjonotapahtuman päätyttyä luokan seuraavan työn, jonka tilana on Odottaa, tilaksi muuttuu Valmis. |
...
Työjonotapahtuman asettaminen **Pidossa**-arvoksi ei vaikuta jo käynnissä olevaan työhön. Kun työ on alkanut, se jatkuu valmistumiseen asti huolimatta työjonotapahtumaan tehdyistä muutoksista, kuten työn pidossa olosta.<br><br>**Pidossa**-tilaa käytetään yleensä estämään työn automaattinen käynnistyminen, kun se saavuttaa aikataulutetun aloitusajan. Sen avulla voit keskeyttää työn väliaikaisesti ennen sen käsittelyn aloittamista. <br><br>Jos sinun on pysäytettävä tai peruutettava suoritettavana oleva työ, voit puuttua prosessiin. Voit esimerkiksi pysäyttää vastaavan istunnon tai prosessin.

### Minkä tahansa työn tilan näyttäminen

1. Valitse ![Lamppu, joka avaa Kerro, mitä haluat tehdä -ominaisuuden.](media/ui-search/search_small.png "Kerro, mitä haluat tehdä") -kuvake, syötä **Työjonon tapahtumat** ja valitse sitten vastaava linkki.
...
* Työjonotapahtuman tunnus suorittaa virheen
